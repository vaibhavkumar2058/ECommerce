...
export default function Discounts() {
      ...
      formatter: (cellContent, row) => {
        return (
          <><button
            ...
            onClick={() => handleView(row.discountId, row.name)}
          >
            View
          </button>
            <button
              className="btn btn-primary btn-xs"
              onClick={() => handleEdit(row.discountId, row)}
            >
              Edit
            </button><button
              className="btn btn-danger btn-xs"
              onClick={() => handleDelete(row.discountId, row.name)}
            >
              Delete
            </button></>
        );
  ...
  const getAllDiscounts = async () => {
    ...
    if (response.payload.title == "Success") {
      setMessageStatus({
        mode: 'success',
        message: 'Discounts Record Fetch Succefully.'
      })
      ...
      setMessageStatus({
        mode: 'danger',
        message: 'Discount Fetch Failed.'
      })
    }
  };

  const getDiscountById = async (id) => {
    const response = await discountById(id);
    if (response.payload.title == "Success") {
